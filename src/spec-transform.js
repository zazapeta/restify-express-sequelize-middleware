...
let property = (attribute, options) => {
  ...
  if (type instanceof Sequelize.BOOLEAN)
    return { type: addNull ? ["boolean", "null"] : "boolean" };
  if (type instanceof Sequelize.INTEGER)
    return { type: addNull ? ["integer", "null"] : "integer", format: "int32" };
  if (type instanceof Sequelize.BIGINT)
    return { type: addNull ? ["integer", "null"] : "integer", format: "int64" };

  if (type instanceof Sequelize.FLOAT || type instanceof Sequelize.REAL) {
    return { type: addNull ? ["number", "null"] : "number", format: "float" };
  }

  if (type instanceof Sequelize.DOUBLE) {
    return { type: addNull ? ["number", "null"] : "number", format: "double" };
  }

  if (type instanceof Sequelize.DECIMAL) {
    return { type: addNull ? ["number", "null"] : "number" };
  }

  if (type instanceof Sequelize.DATEONLY) {
    return { type: addNull ? ["string", "null"] : "string", format: "date" };
  }
  ...
  if (type instanceof Sequelize.TIME) {
    return { type: addNull ? ["string", "null"] : "string" };
  }
  ...
  if (type instanceof Sequelize.JSON || type instanceof Sequelize.JSONB) {
    return { type: "any" };
  }

  if (type instanceof Sequelize.VIRTUAL) {
    return type.returnType
      ? property({ type: type.returnType, allowNull: type.allowNull }, options)
      : { type: addNull ? ["string", "null"] : "string" };
  }
  ...
  console.log(
    `Unable to convert ${type.key || type.toSql()} to a schema property`
  );
